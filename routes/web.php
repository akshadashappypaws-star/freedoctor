...
// Test home.blade.php template with search functionality
...
Route::get('/test-whatsapp-send', function () {
    return response()->file(public_path('../test_send_debug.html'));
});

// Test email verification page
Route::get('/test/email-verification-page', function () {
    return view('test-email-verification');
})->name('test.email.verification.page');

// Test email verification
Route::get('/test/email-verification', function () {
    $user = auth('user')->user();
    if (!$user) {
        return response()->json(['error' => 'Please login as user first']);
    }
    ...
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Custom email verification sent successfully',
...
Route::post('/test/password-reset', function (\Illuminate\Http\Request $request) {
    ...
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Custom password reset email sent successfully',
            'user_email' => $user->email
        ]);
    } catch (\Exception $e) {
        return response()->json([
            'error' => 'Failed to send password reset: ' . $e->getMessage()
        ]);
    }
...
Route::get('/test/referral-flow', function () {
    return view('test-referral-flow');
})->name('test.referral.flow');

// Test WhatsApp conversations
Route::get('/test-conversations', function() {
    // Clear existing test data
    \App\Models\WhatsappConversation::where('phone', 'like', '+test%')->delete();
    ...
    foreach ($conversations as $data) {
        \App\Models\WhatsappConversation::create($data);
    }
    
    return response()->json([
        'message' => 'Sample conversations created successfully!',
        'total_conversations' => \App\Models\WhatsappConversation::count(),
        'test_conversations' => \App\Models\WhatsappConversation::where('phone', 'like', '+test%')->count()
    ]);
});

// Debug conversations data
Route::get('/debug-conversations', function() {
    $controller = new \App\Http\Controllers\Admin\WhatsappConversationController(
    ...
    try {
        ...
        $allConversations = \App\Models\WhatsappConversation::orderBy('created_at', 'desc')->get();
        ...
        return response()->json([
            'total_conversations' => \App\Models\WhatsappConversation::count(),
            'grouped_count' => $groupedConversations->count(),
            'sample_data' => array_slice($conversationSummaries, 0, 3),
            'all_conversations' => $allConversations->take(5)
        ]);
...
// Test routes for withdrawal system (accessible without auth for testing)
Route::get('/test/withdrawal', function () {
    return view('withdrawal-test');
})->name('test.withdrawal');

Route::post('/test/add-balance', function (Request $request) {
    $user = auth('user')->user();
    if (!$user) {
...
})->name('test.add-balance');

// Test withdrawal routes (to debug authentication issues)
Route::post('/test/withdrawal/process', function (Request $request) {
    return response()->json([
        'success' => true,
...
})->name('test.withdrawal.process');

Route::post('/test/withdrawal/account-details', function (Request $request) {
    return response()->json([
        'success' => true,
        'message' => 'Test account details route working',
        'authenticated' => auth('user')->check(),
        'user_id' => auth('user')->id(),
        'request_data' => $request->all()
    ]);
})->name('test.withdrawal.account-details');
