...
<div class="login-card" role="main" aria-label="Doctor Registration Form">
  ...
  <p class="subtitle">login your Doctor account below.</p>

  <div class="tabs" role="tablist" aria-label="Doctor login or Info">
    <button class="tab active" role="tab" aria-selected="true" aria-controls="login-panel" id="login-tab" tabindex="0">Doctor login</button>
...
@endif
      ...
      <form method="POST" action="{{ route('doctor.login') }}">
        ...
        <!-- Google Login Button -->
        <button type="button" class="google-login-btn" onclick="loginWithGoogle('doctor')">
        ...
        <div class="row">
          <div class="col-8"><a href="{{ route('doctor.password.request') }}">Forgot Password?</a></div>
          <div class="col-4"><button type="submit" class="btn btn-primary btn-block">Login</button></div>
        </div>
      </form>
      <p class="mt-3 mb-1">Don't have an account? <a href="{{ route('doctor.registerform') }}">Register</a></p>
